...
async def criar_passageiro(
    passageiro: PassageiroCreate, db: AsyncSession = Depends(get_db)
):
    try:
...
async def criar_voo(voo: VooCreate, db: AsyncSession = Depends(get_db)):
    try:
        service = ViagemService(db)
...
async def criar_passagem(passagem: PassagemCreate, db: AsyncSession = Depends(get_db)):
    try:
        service = ViagemService(db)
