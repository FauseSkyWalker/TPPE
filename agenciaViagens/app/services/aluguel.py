...
from ..schemas import CarroCreate, HotelCreate, AluguelCarroCreate, AluguelHotelCreate
...
class AluguelService:
    ...
    async def criar_carro(self, carro: CarroCreate):
        ...
        if not re.match(r"^[A-Z0-9]{7}$", placa):
            raise ValueError(
                "Placa deve ter exatamente 7 caracteres (letras maiúsculas e números)"
            )
    ...
    async def alugar_carro(self, aluguel: AluguelCarroCreate):
        ...
        for aluguel_existente in alugueis:
            if (
                aluguel.data_inicio <= aluguel_existente.data_fim
                and aluguel.data_fim >= aluguel_existente.data_inicio
            ):
