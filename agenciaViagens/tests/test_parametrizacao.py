...
def test_validacao_usuario(nome, email, senha, esperado):
    ...
    assert Usuario(nome=nome, email=email, senha=senha).validar() == esperado


@pytest.mark.parametrize(
    "cpf,data_nascimento,esperado",
    [
...
def test_validacao_pessoa_fisica(cpf, data_nascimento, esperado):
    # Criar pessoa física e validar
    assert PessoaFisica(cpf=cpf, data_nascimento=data_nascimento).validar() == esperado


@pytest.mark.parametrize(
    "tipo_passageiro,valor_esperado",
    [
...
def test_calculo_valor_passagem(tipo_passageiro, valor_esperado):
    passageiro = Passageiro(
        nome="Test", documento="123", tipo_passageiro=tipo_passageiro
    )
    valor_base = 100.0
    assert (
        calcular_valor_passagem(passageiro, valor_base) == valor_base * valor_esperado
    )
